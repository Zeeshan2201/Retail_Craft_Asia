...
const FeatureCard = ({ icon, title, description, index }) => {
  ...
  const iconVariants = {
    ...
    hover: {
      scale: 1.1,
      rotate: [0, -5, 5, 0],
      transition: { rotate: { duration: 0.5, ease: "easeInOut", times: [0, 0.25, 0.75, 1] } },
    },
  }
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-xl border border-yellow-400 shadow-[0_0_20px_rgba(251,191,36,0.4)]"
    >
      {/* Gradient Card Background */}
      <motion.div
        className="bg-gradient-to-br from-yellow-100 via-orange-50 to-yellow-200 h-full shadow-lg rounded-xl p-8 flex flex-col items-center relative z-10"
        animate={{
          boxShadow: isHovered
            ? "0 20px 25px -5px rgba(245, 158, 11, 0.25), 0 10px 10px -5px rgba(245, 158, 11, 0.15)"
            : "0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)",
        }}
      >
        <motion.div
          className="bg-gradient-to-br from-yellow-50 to-red-100 p-6 rounded-full mb-6 relative"
        ...
        <motion.h3
          ...
          animate={{ color: isHovered ? "#d97706" : "#111827" }}
      ...
      {/* Hover Glow Layer */}
      <motion.div
        className="absolute inset-0 bg-gradient-to-br from-yellow-100 via-orange-100 to-yellow-200 opacity-0 z-0"
  ...
  return (
    <section ref={sectionRef} className="py-20 px-6 bg-gradient-to-b from-white to-gray-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <motion.h2
  className="text-5xl md:text-6xl font-semibold text-[#D18B00] uppercase tracking-wide drop-shadow-md"
  animate={{
    textShadow: [
...
>
  Trusted Growth Partner
          ...
          <motion.div
            ...
            animate={{ width: ["10rem", "16rem", "10rem"] }}
            transition={{ duration: 4, repeat: Number.POSITIVE_INFINITY, repeatType: "reverse" }}
          />
      ...
      {[...Array(6)].map((_, i) => (
        <motion.div
          ...
          animate={{ y: [0, -100, 0], x: [0, Math.random() * 50 - 25, 0], opacity: [0.4, 0.8, 0.4] }}
          transition={{ duration: 10 + Math.random() * 10, repeat: Number.POSITIVE_INFINITY, delay: i * 2 }}
        />
      ))}
    </section>
  )
